...
import rl_sim.gui.state.StateActionInfo;
import rl_sim.gui.state.StateAndVisits;
import rl_sim.gui.state.StateInfo;
...
public class PrioritizedSweeping {//implements rl_sim.backend.algorithms.Algorithms
    ...
    public PriorityQueue pQueue;    //priority queue
...
//		if((deltaQ/qBefore) > 0.4)//change is greater than 40% of original value
    ...
    private void processQueue() {
        //rl_sim.gui.Utility.show("===========>Processing Queue<================");
        int counter = 0;
        while (!pQueue.isEmpty() && counter <= BACKUPS_ALLOWED) {
            //rl_sim.gui.Utility.show("Counter: "+counter);
            //pQueue.printQueue();
            //step 2.1
...
//			currValues.stateValue[top.x][top.y] = model[top.x][top.y].getMinQsa();
        ...
        //rl_sim.gui.Utility.show("===========>Backupstate over<================");
        //rl_sim.gui.Utility.show("");
    }
    ...
    void printBackedUpStates() {
        ...
        State st;
        for (Object backedUpState : backedUpStates) {
            st = (State) backedUpState;
            st.printState();
        }
        System.out.println();
    }
    ...
    public double evalPolicy() {
        ...
        double maxDelta, delta, v, maxV, minV = 10000, prob, safe;
        ...
        while (!valueConverged) {
            ...
            for (int i = 0; i < myMaze.width; i++)
                System.arraycopy(evalVals.stateValue[i], 0, prevEvalVals.stateValue[i], 0, myMaze.height);
        }
